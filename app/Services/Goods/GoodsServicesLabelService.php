 ...
 */
class GoodsServicesLabelService
{
    ...
    public function labelUpdateStatus($data = [], $id = 0)
    {
        ...
        return GoodsServicesLabel::query()->where('id', $id)->update($data);
    }
     ...
     * @return array
     */
    public function getServicesLabelList($filter = [], $offset = [])
    {
        $list = GoodsServicesLabel::query();
        ...
        if (!empty($list)) {
            foreach ($list as $k => $row) {
                ...
                if (!empty($row['bind_goods_number']) && $ru_id == 0) {
                    ...
                    // 商家id
                    $bind_goods_number = GoodsUseServicesLabel::where('label_id', $row['id'])->whereHasIn('getGoods', function ($query) use ($ru_id) {
                        $query->where('user_id', $ru_id);
                    });
                    $bind_goods_number = $bind_goods_number->count('goods_id');
                }

                if ($row['label_code'] == 'no_reason_return') {
                    $bind_goods_number = Goods::query()->where(function ($query) {
                        $query->whereRaw("FIND_IN_SET('1', goods_cause)")->orWhereRaw("FIND_IN_SET('3', goods_cause)");
                    })->count();
                }
    ...
    public function getLabelInfo($id = 0)
    {
        ...
        $row = GoodsServicesLabel::query()->where('id', $id);
    ...
    public function goodsLabelInstall($data = [])
    {
        ...
        /* 过滤表字段 */
        $label = BaseRepository::getArrayfilterTable($data, 'goods_services_label');

        $label['add_time'] = TimeRepository::getGmTime();

        return GoodsServicesLabel::query()->insert($label);
    }
     ...
     * @return bool
     */
    public function goodsServicesLabelUpdate($id = 0, $data = [])
    {
        ...
        GoodsServicesLabel::query()->where('id', $id)->update($label);
    ...
    */
    ...
    {
        $count = GoodsServicesLabel::query()->where('label_name', $label);
     ...
     * @return bool
     */
    public function servicesLabelDrop($id = 0)
    {
        if ($id > 0) {
            GoodsServicesLabel::query()->where('id', $id)->delete(); // 删除服务标签
            GoodsUseServicesLabel::query()->where('label_id', $id)->delete();// 同步商品删除标签
            return true;
        }
        return false;
    }
    ...
    */
    public function batchUpdate($id_list = [], $update = [])
    {
        ...
        GoodsServicesLabel::query()->whereIn('id', $id_list)->update($update);
    ...
    */
    public function batchDrop($id_list = [])
    {
        ...
        GoodsServicesLabel::query()->whereIn('id', $id_list)->delete(); // 删除服务标签
        GoodsUseServicesLabel::query()->whereIn('label_id', $id_list)->delete();// 同步商品删除标签
    ...
    public function bindGoodsList($label_id = 0, $filter = [], $offset = [])
    {
        ...
        $model = GoodsUseServicesLabel::query()->where('label_id', $label_id);
    ...
    public function bind_goods_to_label($label_id = 0, $goods_id_arr = [])
    {
        ...
        foreach ($goods_id_arr as $goods_id) {
            ...
            GoodsUseServicesLabel::query()->updateOrInsert($where, $values);
        }

        GoodsServicesLabelRepository::update_label_bind_goods_number($label_id);
    ...
    public function unbind_goods_to_label($label_id = 0, $goods_id_arr = [])
    {
        ...
        DB::table('goods_use_services_label')->where('label_id', $label_id)->whereIn('goods_id', $goods_id_arr)->delete();
