...
class OrderDeliveryController extends BaseController
{
    ...
    public function order_delivery_download(Request $request, OrderDeliveryService $orderDeliveryService)
    {
        ...
        $result['content'] = $this->fetch('admin/orderdelivery.library.order_delivery_download');
        ...
        if ($request->isMethod('POST')) {
            ...
            if (!empty($form_token)) {
                ...
                if (cache()->has($form_token)) {
                    // 重复请求
                    return $this->message('form repeat submit', null, 2);
                }
            ...
            if ($file && $file->isValid()) {
                ...
                if (!in_array($file->getClientMimeType(), ['application/vnd.ms-excel', 'text/plain'])) {
                    return $this->message(lang('admin/order.not_file_type'), null, 2);
                }
            ...
            if (empty($data['file'])) {
                return $this->message(lang('admin/order.please_upload_excel'), null, 2);
            }
            ...
            if (file_exists($filename)) {
                ...
                if (!empty($res)) {
                    ...
                    $action_user = session('admin_name', '');
                    $admin_id = session('admin_id', 0);
                    ...
                    if (empty($error_order_record)) {
                        return $this->message(lang('admin/order.order_delivery_import_batch') . lang('admin/common.success'), null);
                    } else {
                        ...
                        return $this->message(lang('admin/order.order_delivery_import_batch') . lang('admin/common.fail') . '！' . $msg, null, 2);
                    }
                }

            }

            return $this->message(lang('admin/order.order_delivery_import_batch') . lang('admin/common.fail'), null, 2);
        }

        // 显示
        return $this->display('admin/orderdelivery.order_delivery_import');
    }
