...
class GoodsProdutsAreaBatchController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($act == 'add') {
            ...
            $warehouse_id = request()->get('warehouse_id', 0); //仓库
            $area_id = request()->get('area_id', 0); //地区
            $city_id = request()->get('city_id', 0); //城市
        ...
        elseif ($act == 'upload') {
            ...
            if ($file && $file->isValid()) {
                ...
                $line_number = 0;
                $field_list = array_keys(__('admin::goods_produts_area_batch.upload_product')); // 字段列表
        ...
        elseif ($act == 'download') {
            ...
            $area_id = request()->get('area_id', 0); //当前地区ID;
            $city_id = request()->get('city_id', 0); //当前地区城市;
            ...
            if ($lang != config('shop.lang')) {
                $upload_product = __('admin::goods_produts_area_batch.upload_product', [], $lang);
            } else {
                $upload_product = __('admin::goods_produts_area_batch.upload_product');
            }
            ...
            if (is_array($upload_product)) {
                ...
                if ($lang == 'zh-CN' || $lang == 'zh-TW') {
                    ...
                    if (empty($attribute)) {
                        $link[] = ['href' => 'goods.php?act=edit&goods_id=' . $goods_id, 'text' => __('admin::goods_produts_area_batch.edit_goods')];
                    ...
                    if (config('shop.goods_attr_price') == 1) {
                        if (config('shop.add_shop_price') == 0) {
                            $data .= "," . __('admin::goods_produts_area_batch.market_price');
                        }
                        $data .= "," . __('admin::goods_produts_area_batch.product_price');
                        if (config('shop.add_shop_price') == 0) {
                            $data .= "," . __('admin::goods_produts_area_batch.promote_price');
                        }
                    }

                    $data .= "," . __('admin::goods_produts_area_batch.product_number');
                    $data .= "," . __('admin::goods_produts_area_batch.product_warn_number');
                    $data .= "," . __('admin::goods_produts_area_batch.product_sn');
                    $data .= "," . __('admin::goods_produts_area_batch.bar_code') . "\t\n";
