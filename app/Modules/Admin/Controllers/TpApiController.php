...
class TpApiController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($act == 'kdniao') {
            ...
            $api_config = [];
            $api_config['client_id'] = config('shop.kdniao_client_id');
            $api_config['appkey'] = config('shop.kdniao_appkey');
            $api_config['kdniao_account_use'] = config('shop.kdniao_account_use');
            $this->smarty->assign('api_config', $api_config);

            return $this->smarty->display('tp_api.dwt');
        } //快递鸟更新
        elseif ($act == 'kdniao_update') {
            admin_priv('shop_config');

            $client_id = e(request()->input('client_id', ''));
            $appkey = e(request()->input('appkey', ''));
            $kdniao_account_use = (int)request()->input('kdniao_account_use', 0);

            DB::table('shop_config')->where('code', 'kdniao_client_id')->update(['value' => $client_id]);
            DB::table('shop_config')->where('code', 'kdniao_appkey')->update(['value' => $appkey]);
            DB::table('shop_config')->where('code', 'kdniao_account_use')->update(['value' => $kdniao_account_use]);
