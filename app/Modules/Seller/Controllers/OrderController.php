...
class OrderController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($act == 'list') {
            ...
            $this->smarty->assign('action_link_export', ['href' => route('seller/order/export'), 'text' => lang('seller/common.11_order_export')]);
            ...
            $this->smarty->assign('action_link_delivery_batch', ['href' => route('seller/order_delivery_import'), 'text' => lang('admin/order.order_delivery_import')]);


            $this->smarty->assign('status_list', trans('seller/order.seller_cs'));   // 订单状态
            ...
            $this->smarty->assign('ur_here', lang('seller/common.12_back_apply'));
        ...
        elseif ($act == 'info') {
            ...
            $agency_id = AdminUser::where('user_id', session('seller_id'))->value('agency_id');
            ...
            $order['confirm_take_time'] = $order['confirm_take_time'] > 0 ?
                TimeRepository::getLocalDate(config('shop.time_format'), $order['confirm_take_time']) : ($order['shipping_status'] == 1 ? $GLOBALS['_LANG']['not_confirm_order'] : $GLOBALS['_LANG']['ss'][SS_UNSHIPPED]);
            ...
            if ($comment) {
                ...
                $order['comment_time'] = $comment['add_time'] > 0 ?
                    TimeRepository::getLocalDate(config('shop.time_format'), $order['add_time']) : lang('seller/order.no_comment');
            }
            ...
            if ($order['user_id'] > 0) {
                ...
                // 地址信息
                $res = UserAddress::where('user_id', $order['user_id'])->where('address_id', $user_info['address_id']);
            ...
            /* 取得能执行的操作列表 */
            $operable_list = $this->orderManageService->operableList($order, session('seller_action_list'), $payment);
            ...
            if (isset($_GET['print'])) {
                ...
                $this->assign('shop_url', $this->dsc->seller_url());
                ...
                $this->assign('action_user', session('seller_name'));
        ...
        elseif ($act == 'return_info') {
            ...
            // 退款/退货申请 商家退款是否由平台审批 否
            $refound_agree = 0;
            if (config('shop.seller_return_check', 0) == 0 && in_array($back_order['return_type'], [1, 3])) {
                $refound_agree = 1;
            }
            $this->smarty->assign('refound_agree', $refound_agree);
        ...
        elseif ($act == 'ajax_order_detection') {
            ...
            if (session()->has('is_ajax_detection') && session('is_ajax_detection') == 1) {
                ...
                if ($result['order'] && $orderDelivery && $result['order']['shipping_status'] != SS_RECEIVED) {
                    ...
                    event(new \App\Events\OrderReceiveEvent($result['order'], $extendParam));
                }
        ...
        elseif ($act == 'edit_refound_amount') {
            ...
            if ($type == 1) {
                $order_return = OrderReturn::where('ret_id', $ret_id)->select('should_return', 'goods_coupons', 'goods_bonus', 'goods_favourable');
        ...
        elseif ($act == 'step_post') {
                ...
                if (isset($_POST['next'])) {
                    ...
                    if (!empty($msgs)) {
                        ...
                        if ($shipping_id != $order['shipping_id']) {
                            ...
                            order_action($order['order_sn'], $order['order_status'], $order['shipping_status'], $order['pay_status'], $order_note, session('seller_name'), 0, TimeRepository::getGmTime());
                        }
            ...
            elseif ('payment' == $step) {
                ...
                $this->orderCommonService->orderAction($orderInfo['order_sn'], $orderInfo['order_status'], $orderInfo['shipping_status'], $orderInfo['pay_status'], sprintf(lang('order.edit_pay_status'), $orderInfo['pay_name']), session('seller_name'));
                ...
                if ($is_update) {
                    $this->orderManageService->updateOrderMoneyLog($old_order, $order, session('seller_name'));
                }
                ...
                if (isset($_POST['finish'])) {
                    ...
                    if ($step_act == 'add') {
                        ...
                        if (config('shop.use_storage') == '1' && config('shop.stock_dec_time') == SDT_PLACE) {
                            change_order_goods_storage($order_id, true, SDT_PLACE, config('shop.stock_dec_time'), session('seller_id'));
                        }

                        /* 如果使用库存，且付款时减库存，且订单金额为0，则减少库存 */
                        if (config('shop.use_storage') == '1' && config('shop.stock_dec_time') == SDT_PAID && $order['order_amount'] <= 0) {
                            change_order_goods_storage($order_id, true, SDT_PAID, 15, session('seller_id'));
                        }
                    }
        ...
        elseif ($act == 'add' || $act == 'edit') {
            ...
            if ($order_id > 0) {
                ...
                if ($order && $order['ru_id'] != $adminru['ru_id']) {
                    $Loaction = "order.php?act=list";
                    return dsc_header("Location: $Loaction\n");
                }
            ...
            elseif ('shipping' == $step) {
                ...
                foreach ($shipping_list as $key => $val) {
                    if (substr($val['shipping_code'], 0, 5) != 'ship_') {
                        ...
                        $shipping_list[$key]['shipping_fee'] = $shipping_fee;
                        $shipping_list[$key]['insure_formated'] = empty($val['insure']) ? '' : (isset($val['insure']) && strpos($val['insure'], '%') === false ? $this->dscRepository->getPriceFormat($val['insure'], false) : $val['insure']);
                        $shipping_list[$key]['format_free_money'] = empty($shipping_cfg['free_money']) ? '' : $this->dscRepository->getPriceFormat($shipping_cfg['free_money'], false);
        ...
        elseif ($act == 'operate') {
            ...
            elseif (isset($_POST['to_shipping'])) {
                ...
                $res = $this->orderManageService->oneClickShipping($data, $action_note, 'one', session('seller_id'), session('seller_name'));
                if (!empty($res['delivery_id'])) {
                    /* 操作成功 */
                    $links[] = ['text' => lang('seller/common.09_delivery_order'), 'href' => 'order.php?act=delivery_list'];
                    $links[] = ['text' => lang('seller/order.delivery_sn') . lang('seller/order.detail'), 'href' => 'order.php?act=delivery_info&delivery_id=' . $res['delivery_id']];
            ...
            elseif (isset($_POST['ship'])) {
                ...
                $order['shipping_time'] = $order['shipping_time'] > 0 ?
                    TimeRepository::getLocalDate(config('shop.time_format'), $order['shipping_time']) : $GLOBALS['_LANG']['ss'][SS_UNSHIPPED];
                $order['confirm_time'] = TimeRepository::getLocalDate(config('shop.time_format'), $order['confirm_time']);
                /* 查询：是否保价 */
                $order['insure_yn'] = empty($order['insure_fee']) ? 0 : 1;
            ...
            elseif (isset($_POST['return'])) {
                ...
                if ($ret_id > 0) {
                    ...
                    return sys_msg($GLOBALS['_LANG']['order_have_return_cant_return'], 1, $links);
                } else {
                ...
                $res = ValueCardRecord::where('order_id', $order['order_id'])->select('vc_id', 'use_val');
                $value_card = BaseRepository::getToArrayFirst($res);
                ...
                if (config('shop.seller_return_check', 0) == 1) {
                    // 商家退款是否由平台审批 => 是 则由商家提交退款申请后 待平台审批退款
                    $action = trans('seller::order.return_apply');
                    $this->smarty->assign('return_apply', 1);
                } else {
                    // 商家退款是否由平台审批 => 否 则由商家操作退款
                    // 优先退款方式 1 退回会员余额, 2 线下退款, 6 原路退款
                    $precedence_return_type = config('shop.precedence_return_type', 6);
            ...
            elseif (isset($_POST['refound'])) {
                ...
                $shipping = request()->input('return_shipping_fee', 0);
                ...
                } elseif ($shippingFee == 0 && $is_refound_shippfee == 0) {
                ...
                if ($return_order['goods_coupons'] > 0) {
                    $refund_amount = $refund_amount - $return_order['goods_coupons'];
                }

                if ($return_order['goods_bonus'] > 0) {
                    $refund_amount = $refund_amount - $return_order['goods_bonus'];
                }

                if ($return_order['goods_favourable'] > 0) {
                    $refund_amount = $refund_amount - $return_order['goods_favourable'];
                }
                ...
                if ($value_card) {
                    ...
                    if (!empty($value_card['use_val'])) {
                        if ($order['vc_dis_money'] > 0) {
                            $refund_amount -= $order['vc_dis_money'];
                        }

                        $refund_amount = $refund_amount - $value_card['use_val']; //减去储值卡金额
                    }
                }
                ...
                /**
                 * 商家退款是否由平台审批 => 否 同意申请且同时退款
                 */
                $refound_agree = (int)request()->input('refound_agree', 0);
                if (config('shop.seller_return_check', 0) == 0 && $refound_agree == 1) {
                    // 返回链接
                    $links[] = [
                    ...
                    try {
                        ...
                        return sys_msg($httpException->getMessage(), 1, $links);
                    }
                    ...
                    $refund_type = $precedence_return_type;

                    $refound_vcard = $value_card['use_val'] ?? 0;
                    $rate_price = $return_order['return_rate_price'] ?? 0;
                    ...
                    // 退款
                    try {
                        $respond = $this->orderRefoundService->operatePostRefound($rec_id, $ret_id, $order, $refund_type, $rate_price, $refund_note, $action_note, $operation);
                    } catch (HttpException $httpException) {
                    ...
                    if ($respond) {
                        ...
                        /* 记录log */
                        $this->orderCommonService->returnAction($ret_id, RF_COMPLETE, FF_REFOUND, $action_note, session('seller_name'));
            ...
            elseif (isset($_POST['remove'])) {
                ...
                if (!$batch) {
                    ...
                    $operable_list = $this->orderManageService->operableList($order, session('seller_action_list'));
                    ...
                    if ($return_order) {
                        return sys_msg(sprintf(lang('seller/order.order_remove_failure'), $order['order_sn']), 1, [['href' => 'order.php?act=list&' . list_link_postfix(), 'text' => lang('seller/order.return_list')]]);
                    }
        ...
        elseif ($act == 'batch_operate_post') {
                ...
                foreach ($order_id_list as $id_order) {
                    ...
                    if ($order) {
                        ...
                        if (config('shop.send_confirm_email', 0) == '1') {
                            ...
                            $this->smarty->assign('order', $order);
                            $this->smarty->assign('shop_name', config('shop.shop_name', ''));
                            $this->smarty->assign('send_date', TimeRepository::getLocalDate(config('shop.time_format')));
                            $this->smarty->assign('sent_date', TimeRepository::getLocalDate(config('shop.time_format')));
                            $content = $this->smarty->fetch('str:' . $tpl['template_content']);
            ...
            if (empty($sn_not_list)) {
                ...
                foreach ($res as $row) {
                    ...
                    $order_list_fail = '';
                    $operable_list = $this->orderManageService->operableList($row, session('seller_action_list'));
        ...
        elseif ($act == 'operate_post') {
            ...
            elseif ('pay' == $operation) {
                ...
                if (config('shop.use_storage') == '1' && config('shop.stock_dec_time') == SDT_PAID) {
                    change_order_goods_storage($order_id, true, SDT_PAID, 15, session('seller_id'), $store_id);
                }
                ...
                if ($order['extension_code'] == 'presale' && $order['pay_status'] == 0) {
                    order_action($order['order_sn'], $arr['order_status'], $order['shipping_status'], PS_PAYED_PART, $action_note, session('seller_name'));
                    /* 更新 pay_log */
                    $this->orderCommonService->updateOrderPayLog($order_id);
                } else {
                    order_action($order['order_sn'], $arr['order_status'], $order['shipping_status'], PS_PAYED, $action_note, session('seller_name'), 0, $confirm_take_time);
                }

                /* 更新主订单付款状态 */
                $this->orderCommonService->updateMainOrder($order, 2, $action_note, session('seller_name'));
            ...
            elseif ('split' == $operation) {
                ...
                if (true) {
                    /* 获取表单提交数据 */
                    $_REQUEST['send_number'] = isset($_REQUEST['send_number']) ? $_REQUEST['send_number'] : [];
                    ...
                    if ($order['order_status'] == OS_SPLITED) {
                        /* 操作失败 */
                        $links[] = ['text' => lang('seller/order.order_info'), 'href' => 'order.php?act=info&order_id=' . $order_id];
                        return sys_msg(sprintf($GLOBALS['_LANG']['order_splited_sms'], $order['order_sn'], $GLOBALS['_LANG']['os'][OS_SPLITED], $GLOBALS['_LANG']['ss'][SS_SHIPPED_ING], config('shop.shop_name', '')), 1, $links);
                    }
            ...
            elseif ('receive_goods' == $operation) {
                ...
                if ($res) {
                    ...
                    /* 记录log */
                    $this->orderCommonService->returnAction($ret_id, RF_RECEIVE, '', $action_note, session('seller_name'));
                }
            }
            ...
            elseif ('swapped_out_single' == $operation) {
                ...
                $res = OrderReturn::where('rec_id', $rec_id)->update($arr);
                if ($res) {
                    $this->orderCommonService->returnAction($ret_id, RF_SWAPPED_OUT_SINGLE, '', $action_note, session('seller_name'));
                }
            }
            ...
            elseif ('swapped_out' == $operation) {
                ...
                $res = OrderReturn::where('rec_id', $rec_id)->update($arr);
                if ($res) {
                    $this->orderCommonService->returnAction($ret_id, RF_SWAPPED_OUT, '', $action_note, session('seller_name'));
                }
            }
            ...
            elseif ('refuse_apply' == $operation) {
                ...
                $res = OrderReturn::where('rec_id', $rec_id)->update($arr);
                if ($res) {
                    $this->orderCommonService->returnAction($ret_id, REFUSE_APPLY, '', $action_note, session('seller_name'));
                }
            }
            ...
            elseif ('complete' == $operation) {
                ...
                $res = OrderReturn::where('rec_id', $rec_id)->update($arr);
                if ($res) {
                    $this->orderCommonService->returnAction($ret_id, RF_COMPLETE, $refound_status, $action_note, session('seller_name'));
                }
            }
            ...
            elseif ('refound' == $operation) {
                ...
                if ($respond) {
                    ...
                    // 收银台代码 推送核销状态到收银台
                    event(new PickupOrdersUpdateAfterDoSomething(['order_id' => $order_id]));
            ...
            elseif ('return' == $operation) {
                ...
                if ($order['pay_status'] != PS_UNPAYED) {
                    ...
                    $refund_note = "【" . lang('seller/order.refund') . "】" . "【" . $order['order_sn'] . "】" . $action_note;

                    $return_apply = (int)request()->input('return_apply', 0); // 是否生成退款申请 0 否、1 是
                    ...
                    try {
                        ...
                        $this->orderRefoundService->submitReturn($order['user_id'], $request_info, session('seller_name'));
                    } catch (HttpException $httpException) {
                        /* 提示信息 */
                        return sys_msg(lang('admin/order.operation_error') . ',' . $httpException->getMessage(), 1, $links);
                    }
                    ...
                    if (!empty($return_list)) {
                        foreach ($return_list as $order_return) {
                            ...
                            if ($r) {
                                // 记录log 同意申请
                                $this->orderCommonService->returnAction($order_return['ret_id'], RF_AGREE_APPLY, '', $action_note, session('seller_name'));
                            }
                        }
                    }

                    if (config('shop.seller_return_check', 0) == 1 && $return_apply == 1) {
                        // 商家订单 平台开启审批 仅生成退款申请  待平台审批
                        $links = [
                            ['text' => lang('seller/order.order_info'), 'href' => 'order.php?act=info&order_id=' . $order_id],
                ...
                if (config('shop.use_storage') == '1') {
                    if (config('shop.stock_dec_time') == SDT_SHIP) {
                        ...
                        change_order_goods_storage($order['order_id'], false, SDT_PAID, 6, session('seller_id'), $store_id);
                    }
                }
                ...
                if ($order['shipping_status'] == SS_SHIPPED) {
                    ...
                    if ($delivery_list) {
                        foreach ($delivery_list as $list) {
                            ...
                            if ($res) {
                                //获取退换货商品ID
                                $res = BackGoods::where('back_id', $back_id)->select('goods_id', 'send_number');
                                $res = BaseRepository::getToArrayFirst($res);
            ...
            elseif ('after_service' == $operation) {
                ...
                /* 记录log */
                order_action($order['order_sn'], $order['order_status'], $order['shipping_status'], $order['pay_status'], '[' . lang('admin/order.op_after_service') . '] ' . $action_note, session('seller_name'));
            } else {
                return sys_msg('invalid params', 1);
            }
        ...
        elseif ($act == 'merge') {
            ...
            if ($merchant_id > 0) {
                ...
                $where = " AND o.ru_id = '$merchant_id' AND o.main_count = 0 ";
        ...
        elseif ($act == 'ajax_merge_order_list') {
            ...
            return make_json_result($this->smarty->fetch('merge_order.dwt'));
        }
        ...
        elseif ($act == 'ajax_merge_order') {
            ...
            // from_order_sn: o2018051518208788790  to_order_sn: o2018051518208788790
            ...
            if (!empty($from_order_sn)) {
                $from_order_sn = substr($from_order_sn, 1);
        ...
        elseif ($act == 'remove_order') {
            ...
            $return_order = OrderReturn::query()->where('order_id', $order_id)->count();
            if ($return_order) {
                return make_json_error(sprintf(trans('seller/order.order_remove_failure'), $order['order_sn']));
            }

            // 检查订单是否有未处理投诉
            $complaint_order = DB::table('complaint')->where('order_id', $order['order_id'])->where('complaint_state', '<>', 4)->count('complaint_id');
            if ($complaint_order) {
        ...
        elseif ($act == 'edit_invoice_no') {
            ...
            $messages = [
                ...
                'val.required' => trans('seller::order.invoice_no_sms'),
                'val.regex' => trans('seller::order.invoice_no_notice')
            ];
            ...
            if ($res > 0) {
                if (!empty($invoice_no) && $invoice_no != $order['invoice_no']) {
                    ...
                    if ($up) {
                        ...
                        $note = sprintf(lang('seller/order.edit_order_invoice'), $order['invoice_no'], $invoice_no);
        ...
        elseif ($act == 'key_delivery') {
            ...
            $action_note = $action_note ?? '';
            $res = $this->orderManageService->oneClickShipping($order_arr, $action_note, 'one', session('seller_id'), session('seller_name'));
            if (!empty($res['delivery_id'])) {
                /* 操作成功 */
        ...
        elseif ($act == 'key_delivery_many_order') {
            ...
            $res = $this->orderManageService->oneClickShipping($order_arr, $action_note, 'many', session('seller_id'), session('seller_name'));
            $result['error'] = $res['error_code'] ?? 0;
            $result['msg'] = $res['msg'] ?? '';
            return response()->json($result);
        }
    ...
    private function delivery_list()
    {
        ...
        $sql = "SELECT do.delivery_id, do.delivery_sn, do.order_sn, do.order_id, do.add_time, do.action_user, do.consignee, do.country,
                   do.province, do.city, do.district, do.tel, do.status, do.update_time, do.email, do.suppliers_id, o.shipping_status
        ...
        foreach ($row as $key => $value) {
            ...
            $ru_id = $ru_id ? $ru_id : 0;
            $row[$key]['ru_name'] = $this->merchantCommonService->getShopName($ru_id, 1);

        }

        return ['delivery' => $row, 'filter' => $filter, 'page_count' => $filter['page_count'], 'record_count' => $filter['record_count']];
    }
