 ...
 * @method static getToArray($query) 模型对象结果集转换成数组
 ...
 * @method static getGroupBy($list = [], $val = '') 获取组合数组的新数组[指定值为key分组新数组]
 ...
 * @method static getArrayIntersect($list = [], $columns = []) 数组的「值」进行比较，计算数组的交集
 * @method static getArrayIntersectByKeys($list = [], $columns = []) 数组的「键」进行比较，计算数组的交集
 ...
 * @method static keepSortDescKeys($list = [], $sort = '', $extend = '') 保持key值排序[从大到小排序]
 ...
 * @method static getArrayPrepend($list = [], $str = '') 指定的值添加的集合的开头
...
class BaseRepository extends Base
{
    ...
    /**
     * 根据key[键]或者value[值]查找新的数组集合
     *
     * condition ['<', '<=', '>', '>=', '=', 'In', 'notIn', 'betweenIn', 'betweenNotIn']
     *
     * $arr = [
     *      1 => ['a', 'b'],
     *      2 => ['c', 'd'],
     ...
     * ];
     *
     * 示例：$this->getKeyOrValueFilter($arr, $where);
     *
     * 返回结果：
     * array:7 [▼
     *  2 => array:2 [▼
     ...
     * ]
     *
     * 示例：$this->keepSortKeys($list, 'attr_info', 'sort_order');
     *
     * $list = array:3 [▼
     *     0 => array:19 [▼
     *           "goods_attr_id" => 72
     ...
     *       ]
     * ]
     * 示例：$this->keepSortKeys($list, 'attr_sort');
     */
     ...
     *    ]
     * ];
     * $list = BaseRepository::valueErrorArray($arr, 'val', 'id', $default); //处理数组中的val值之和不能大于或者小于$default值
     */
}
