...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    public function checkAuth($appid = '', $appsecret = '', $token = '')
    {
        ...
        $cache_id = 'wechat_access_token' . $appid;
    ...
    public function resetJsTicket($appid = '')
    {
        ...
        $this->jsapi_ticket = '';
        $cache_id = 'wechat_jsapi_ticket' . $appid;
        $this->removeCache($cache_id);
        return true;
    }
    ...
    public function getJsTicket($appid = '', $jsapi_ticket = '')
    {
        ...
        $cache_id = 'wechat_jsapi_ticket' . $appid;
        if (($jsapi_ticket = $this->getCache($cache_id)) && !empty($jsapi_ticket)) {
            $this->jsapi_ticket = $jsapi_ticket;
            return $this->jsapi_ticket;
        }
        ...
        if ($result) {
            ...
            $expire = $json['expires_in'] ? intval($json['expires_in']) - 300 : 7200;
            $this->setCache($cache_id, $this->jsapi_ticket, $expire);
            return $this->jsapi_ticket;
        }
    ...
    public function getJsCardTicket($appid = '', $api_ticket = '')
    {
        ...
        $cache_id = 'wechat_api_ticket_wxcard' . $appid;
        if (($api_ticket = $this->getCache($cache_id)) && !empty($api_ticket)) {
            return $this->api_ticket = $api_ticket;
        }
        ...
        if ($result) {
            ...
            $expire = $json['expires_in'] ? intval($json['expires_in']) - 300 : 7200;
            $this->setCache($cache_id, $this->api_ticket, $expire);
            return $this->api_ticket;
        }
        return false;
    }
    ...
    /**
     * 获取微信服务器IP地址列表
     * @return bool|array('127.0.0.1','127.0.0.1')
     */
    ...
    /**
     * 获取菜单(认证后的订阅号可用)
     * @return bool|array('menu'=>array(....s))
     */
     ...
     * )
     */
    public function getForeverList($type = '', $offset = 0, $count = 1)
    {
        if (!$this->access_token && !$this->checkAuth()) {
            return false;
        }
     ...
     * @return bool|array('ticket'=>'qrcode字串','expire_seconds'=>604800,'url'=>'二维码图片解析后的地址')
     */
