...
//                 $GLOBALS['smarty']->assign('pager', $pager);
    ...
    private function _requestUri($url)
    {
        if (isset($url)) {
            ...
            if (request()->server('argv')) {
                $uri = request()->server('PHP_SELF') . '?' . request()->server('argv')[0];
            } else {
                $uri = request()->server('PHP_SELF') . '?' . request()->server('QUERY_STRING');
            }
        }

        if (request()->server('HTTPS')) {
            $protocol = (request()->server('HTTPS') && (strtolower(request()->server('HTTPS')) != 'off')) ? 'https://' : 'http://';
        } elseif (request()->server('HTTP_X_FORWARDED_PROTO')) {
            $protocol = (request()->server('HTTP_X_FORWARDED_PROTO') && (strtolower(request()->server('HTTP_X_FORWARDED_PROTO')) != 'off')) ? 'https://' : 'http://';
        } else {
