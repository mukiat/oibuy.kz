...
$(function() {
    ...
    var i = 0
    $(".ecsc-nav-ul *[ectype='li']").each(function(){
    ...
    if(i == 0){
        $("*[ectype='moreLi']").addClass("current")
    }else{
        $("*[ectype='moreLi']").removeClass("current")
    }
...
jQuery.category = function(){
	...
	$(document).on('click', '.select-list li', function(){
		...
		$.jqueryAjax('get_ajax_content.php', 'act=filter_category&cat_id='+cat_id+"&cat_type_show=" + cat_type_show + "&user_id=" + user_id + "&table=" + table+ "&level=" + level, function(data){
			if(data.content){
