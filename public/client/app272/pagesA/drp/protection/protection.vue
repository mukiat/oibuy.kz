			...
			<swiper class="swiper-thumbs" previous-margin="120" next-margin="120" display-multiple-items="1" :duration="300" :current="current" @change="swiperTopChange">
				<swiper-item v-for="(item,index) in drpProtection.user_membership_card_rights_list" :key="index">
					<view class="swiper-item-warp">
						<view class="icon"><image :src="item.icon" class="img" mode="widthFix"></image></view>
						<text>{{item.name}}</text>
					</view>
				</swiper-item>
			</swiper>
		...
		methods: {
			...
			makePhone(val){
				if(val){
					...
					uni.showToast({ title: this.$t('lang.fill_in_number_is_not_set'), icon:'none' });
				}
			},
		},
		watch:{
...
.swiper-thumbs .icon{ width: 180rpx; height: 180rpx; }
.swiper-thumbs .icon .img{ border-radius: 50%; }
.swiper-thumbs text{ color: #FFFFFF; margin-top: 10upx; font-size: 28rpx;}
