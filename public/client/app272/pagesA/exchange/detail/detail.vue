<template>
	<view class="container fixed-bottom-padding">
			...
			<view class="goods-wrap">
				<view class="banner">
					<swiper indicator-dots="true" @change="swiperChange">
						...
						<block v-if="gallerylList && gallerylList.length > 0">
				...
				<view class="cont-box">
					<view class="goods-price">
						...
						<text class="price-label">{{$t('lang.integral')}}</text>
						<text class="price-favour" v-if="goodsInfo.market_price">{{$t('lang.label_market_price')}}{{ goodsInfo.market_price }}</text>
					</view>
					<view class="title">
						...
						<view class="goods_outer">
							<view class="text-left" v-if="goodsInfo.sales_volume">{{$t('lang.sales_volume')}} {{ goodsInfo.sales_volume }}</view>
							<view class="text-right" v-if="goodsInfo.goods_number">{{$t('lang.current_stock')}} {{ goodsInfo.goods_number }}</view>
						</view>
					</view>
				</view>
				...
				<view class="uni-card uni-card-not">
					<view class="uni-list">
						<view class="uni-list-cell uni-collapse">
							<view class="uni-list-cell-navigate" :class="[listshow ? 'uni-active' : '']" hover-class="uni-list-cell-hover" @click="trigerCollapse()">
		...
		<view class="btn-goods-action">
			...
			<view class="btn-bar">
				<view class="btn btn-red" @click="onExchange">{{$t('lang.immediately_change')}}</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			appShare() {
				let shareInfo = {
					href: `${this.$websiteUrl}bargain/detail/${this.goods_id}?parent_id=${uni.getStorageSync("user_id")}&platform=APP`,
					title:this.goodsInfo.goods_name,
			...
			onGoodsShare() {
				if (this.$isLogin()) {
					...
					let price = this.goodsInfo.exchange_integral.toString();
					let o = {}
					...
					o = {
						...
						type: 0,
						platform: 'MP-WEIXIN',
						extension_code:'exchange',
						code_url:'pagesA/exchange/detail/detail',
						scene:`${this.goods_id}`,
					...
					o = {
						...
						platform: 'APP',
						extension_code:'exchange',
						code_url:`${this.$websiteUrl}bargain/detail/${this.goods_id}?parent_id=${uni.getStorageSync("user_id")}&platform=APP`,
						thumb:this.gallerylList[0].img_url,
					}
					// #endif
