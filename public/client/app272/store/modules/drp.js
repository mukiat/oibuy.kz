...
const mutations = {
    ...
    [DRP_TEAM](state, o) {
		if(o.page == 1){
		    ...
		    if(o.data.length != 0){
				for(let i= 0;i<o.data.length;i++){
					state.drpTeamData.push(o.data[i])
				}
		    }
		}
    },
    ...
    [DRP_OFFLINE_USER](state, o) {
		if(o.page == 1){
		    ...
		    if(o.data.length != 0){
				for(let i= 0;i<o.data.length;i++){
					state.drpOffkineUserData.push(o.data[i])
				}
		    }
		}
    },
...
const actions = {
   // 开店
	...
	// 分销管理-自定义设置数据
	getCustomText({commit},o){
		return apiDrp.getCustomTextByCode(o)
	}
}
export default {
    state,
