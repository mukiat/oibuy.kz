<template>
	<view class="container fixed-bottom-padding" v-if="checkoutDisplay">
		<view class="title_on">
			<view @click="shop_kud">{{$t('lang.express_delivery')}}</view>
			<view class="active_on">{{$t('lang.private_store')}}</view>
		</view>
		<view class="add_title" @click="shopShow = true" v-if="isSingle == ''">
			<image src="../../static/logo.png"></image>
			<text>{{storepick}}</text>
			<uni-icons type="forward" size="18" color="#999999" class="add"></uni-icons>
		</view>
		<view class="flow-checkout-adr bottom_b" @click="checkoutAdd" v-if="stor == 0">
			<image src="../../static/applocation.png" class="xunadd"></image>
			<view class="adr-content">
				<view class="tit_jin">您已选地区附近没有匹配到合适的门店，请重新更换地区/门店，或切换使用快递配送下单自提地区</view>
			</view>
			<uni-icons type="forward" size="18" color="#999999"></uni-icons>
		</view>
		<view class="flow-checkout-adr flow-store-adr bottom_b" v-else @click="checkoutAdd">
			<uni-icons type="location" size="28" color="#999999"></uni-icons>
			<view class="store-content">
				<view class="tit">{{ storeInfo.stores_name }}</view>
				<text>{{$t('lang.label_service_call')}} {{ storeInfo.stores_tel }}</text>
				<text>{{$t('lang.label_business_hours')}} {{ storeInfo.stores_opening_hours }}</text>
				<text>{{$t('lang.label_store_address')}} [{{ storeInfo.address }}] {{ storeInfo.stores_address }}</text>
			</view>
			<uni-icons type="forward" size="18" color="#999999"></uni-icons>
		</view>
				...
				<view class="uni-list-cell" v-if="checkoutInfo.use_surplus > 0 && use_surplus_val > 0 && pay_code == 'onlinepay'">
					<view class="uni-list-cell-navigate">
						...
						<view class="value uni-cell-input cell-value-flex">
							<input :placeholder="$t('lang.fill_in_use_balance')" type="digit" v-model="surplus" @change="surplusvValHandle"
							 class="input-bor" />
		...
		<!--门店-->
		<uni-popup :show="shopShow" type="bottom" mode="fixed" v-on:hidePopup="shopShow = false">
			<view class="activity-popup">
				<view class="title">
					<view class="txt">选择店铺</view>
					<uni-icons type="closeempty" size="36" color="#999999" @click="shopShow = false"></uni-icons>
				</view>
				<view class="not-content">
					<scroll-view :scroll-y="true" class="select-list">
						<view class="select-item" v-for="(item,index) in actVal" :key="index" :class="{'active': store_ids == item.store_id}"
	...
	export default {
		...
		methods: {
			...
			shop_kud() {
				if (this.goods_id) {
					uni.redirectTo({
						url: '/pages/checkout/checkout?goods_id=' + this.goods_id + '&store_id=' + this.store_id + '&rec_id=' + this.rec_id+ '&ru_id=' + this.ru_id + '&spec_arr=' + this.spec_arr + '&num=' + this.num+ '&isSingle=' + this.isSingle + '&type=true'
					})
				}else{
					uni.redirectTo({
						url: '/pages/checkout/checkout?rec_id=' + this.rec_id + '&ru_ids=' + this.ru_id + '&store_ids=' + this.store_id + '&type=true'
					})
				}
			},
			//购物车列表
			...
			checkoutAdd() {
				if (this.rec_id.length > 0) {
					uni.navigateTo({
						url: '/pagesC/store/store?rec_id=' + this.rec_id
					})
				} else if (this.goods_id) {
					uni.navigateTo({
						url: '/pagesC/store/store?id=' + this.goods_id + "&attr_id=" + this.attr_id + '&num=' + this.num
					})
			...
			store_method_select(id, name, user_id, index) {
				if (id != this.ru_id) {
					...
					uni.navigateTo({
						url: '/pagesC/store/store?rec_id=' + this.rec_id + '&ru_id=' + id
					})
				}
			},
			//默认加载
		...
		onLoad(e) {
			...
			this.ru_id = e.ru_id;
			this.stor = e.stor ? e.stor : 0;
			this.goods_id = e.goods_id ? e.goods_id : '',
			this.attr_id = e.spec_arr ? e.spec_arr : '';
