<template>
    <div class='title'  style="position:relative">
        <header class="transition" :style="{'text-align':sPosition,'background-color':bgColor}" v-if="bStyleSelTradition || bSimplify">
    ...
    computed: {
        ...
        title() {
            return this.getText({
                dataNext: "allValue",
                attrName: "title",
                defaultValue: this.$t('lang.edit_headline_name')
            })
        },
        desc() {
            if (this.bList) {
                return this.getText({
                    listIndex: 0,
                    attrName: "desc",
                    defaultValue: this.$t('lang.edit_url_name')
                })
            }
