							...
							<div class="activity_item" v-for="(activityItem, activityIndex) in formatDiscountsData" :key="activityIndex">
								<template v-if="activityIndex < 2">
									...
									<template v-if="activityItem.acttype == 0">
										<span class="coupon_bg_wrap" v-for="(item, index) in activityItem.value" :key="index"><span class="coupon_bg">{{ item.cou_man }} {{$t('lang.man')}} {{item.cou_money}} {{$t('lang.minus')}}</span></span>
									</template>
								</template>
								<template v-else>
            ...
            goods_promotion(){
                ...
                if(this.goodsInfo.consumption && this.goodsInfo.consumption.length > 0){
                    ...
                    this.goodsInfo.consumption.forEach(v=>{
                        str = v.cfull + this.$t('lang.man') + v.creduce + this.$t('lang.minus')
                        consumption += str + ','
                    })
