  ...
  <div class='menu'>
    <header>
      ...
      <nav class="header-nav">
        <ul>
          <li class="header-btn" v-for="(item, index) in menuComponent" :key="index" v-show="item.isShow && (item.type == 'all' || item.type == sType || item.type == hType)" @mouseenter="navHoverTab(index,$event)" @mouseleave.stop="navOutTab(index)">
...
export default {
  ...
  computed: {
    ...
    hType(){
      return this.pagesName == 'system' && window.shopInfo.ruid == 0 ? "platform-home" : "platform"
    }
  }
}
