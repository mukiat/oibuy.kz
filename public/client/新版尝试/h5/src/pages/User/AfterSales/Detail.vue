  ...
  <div class="con_main">
                ...
                <li>
                  <div class="product-div">
                    ...
                    <div class="product-info">
                      ...
                      <div class="price">
                        <label v-html="refoundDetail.goods_price"></label>
                        <span>x{{refoundDetail.return_number}}</span>
                      </div>
					  <div class="price" v-if="refoundDetail.goods_coupons > 0">- <label v-html="refoundDetail.formated_goods_coupons"></label>[{{ $t('lang.coupons') }}]</div>
					  <div class="price" v-if="refoundDetail.goods_bonus > 0">- <label v-html="refoundDetail.formated_goods_bonus"></label>[{{ $t('lang.bonus') }}]</div>
					  <div class="price" v-if="refoundDetail.goods_favourable > 0">- <label v-html="refoundDetail.formated_goods_favourable"></label>[{{ $t('lang.discount') }}]</div>
					  <div class="price" v-if="refoundDetail.value_card_discount > 0">- <label v-html="refoundDetail.formated_value_card_discount"></label>[{{ $t('lang.value_card_discount') }}]</div>
					  <div class="price" v-if="refoundDetail.goods_coupons > 0 || refoundDetail.goods_bonus > 0 || refoundDetail.goods_favourable > 0 || refoundDetail.value_card_discount > 0">{{ $t('lang.return_total') }}：<label v-html="refoundDetail.formated_should_return"></label></div>
    ...
    <section class="section-list">
    	...
    	<ul class="user-refound-box">
            <li>
                <div>{{ $t('lang.order_sn') }}:</div>
                <div class="value color-red">{{refoundDetail.order_sn}}</div>
            </li>
    		<li>
    	...
    	<ul class="user-refound-box">
    		<li>
    			...
    			<div class="value color-red">{{refoundDetail.return_status1}} - {{refoundDetail.refound_status1}}</div>
    		</li>
    		<li>
            ...
            <template v-if="refoundDetail.return_type == 1 || refoundDetail.return_type == 3">
                ...
                <template v-if="refoundDetail.refound_status == 1">
                    ...
                    <li>
                        ...
                        <div class="value">
                            <div class="price">
                                <label>+ <span v-html="refoundDetail.formated_return_shipping_fee"></span></label>
                            </div>
                        </div>
                    </li>
                    </template>
                    <template v-if="refoundDetail.goods_coupons > 0 ">
                        <li>
                            <label>{{ $t('lang.coupons') }}:</label>
                            <div class="value">
                                <div class="price">
                                    <label>- <span v-html="refoundDetail.formated_goods_coupons"></span></label>
                                </div>
                            </div>
                        </li>
                    </template>
                    <template v-if="refoundDetail.goods_bonus > 0 ">
                    <li>
                        <label>{{ $t('lang.bonus') }}:</label>
                        <div class="value">
                            <div class="price">
                                <label>- <span v-html="refoundDetail.formated_goods_bonus"></span></label>
                            </div>
                        </div>
                    </li>
                    </template>
                    <template v-if="refoundDetail.actual_return > 0">
                    <li>
                        <label>{{ $t('lang.actual_return') }}:</label>
                        <div class="value">
                            <div class="price">
                                <label v-html="refoundDetail.formated_actual_return"></label>
                            </div>
                        </div>
                    </li>
                    </template>
                ...
                <li>
                    ...
                    <div class="value"><div class="price"><label v-html="refoundDetail.formated_should_return"></label></div></div>
                </li>
				<li v-if="refoundDetail.return_shipping_fee > 0">
				    <label>{{ $t('lang.return_shipping') }}:</label>
				    <div class="value"><div class="price"><label v-html="refoundDetail.formated_return_shipping_fee"></label></div></div>
				</li>
				<li v-if="refoundDetail.return_shipping_fee > 0">
				    <label>{{ $t('lang.return_total') }}:</label>
				    <div class="value"><div class="price"><label v-html="refoundDetail.formated_return_amount"></label></div></div>
				</li>
                </template>
            </template>
        ...
        <template v-if="refoundDetail.agree_apply == 1">
            <div class="detail-title m-top10">{{$t('lang.return_shop_address')}}</div>
        	<ul class="user-refound-box">
        		<li>
        			<label>{{$t('lang.consignee')}}:</label>
        			<div class="value">{{refoundDetail.shop_address.contact}}</div>
        		</li>
        		<li>
        			<label>{{$t('lang.phone_number')}}:</label>
        			<div class="value">{{refoundDetail.shop_address.mobile}}</div>
        		</li>
        		<li>
    ...
    methods:{
        ...
        receivedOrder(){
            Dialog.confirm({
                ...
                this.$http.post(`${window.ROOT_URL}api/refound/affirm_receive`,qs.stringify({
                    ret_id:this.$route.query.ret_id
                })).then(({data:{data}})=>{
                    Toast(data.msg)
                    if(data.code == 0){
        ...
        submitBtn(){
            ...
            this.$http.post(`${window.ROOT_URL}api/refound/edit_express`,qs.stringify(o)).then(({data:{data}})=>{
                Toast(data.msg)
                if(data.code == 0){
                    this.refoundLoad()
                }
