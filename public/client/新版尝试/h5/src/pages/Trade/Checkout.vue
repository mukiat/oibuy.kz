<template>
	<div class="con con_main">
		<div class="title_on" v-if="shopStore">
			<div class="active_on">{{$t('lang.express_delivery')}}</div>
			<div @click="shop_kud">{{$t('lang.private_store')}}</div>
		</div>
		<div class="flow-checkout" v-if="checkoutDisplay">
			<section class="flow-checkout-item flow-checkout-adr m-b10 quanymagin" v-if="total.real_goods_count > 0">
				<van-cell-group>
					<van-cell icon="location" is-link @click="checkoutAddress">
						<template slot="title">
							<div class="address_title">{{consignee_title}}</div>
							<div class="address_body">
								<div>
									<div class="post_lebal_red" v-if="checkoutInfo.consignee.post_mobile">{{$t('lang.community_post')}}</div>{{consignee_address}}
								</div>
								<div v-if="checkoutInfo.consignee.post_mobile">{{$t('lang.post_contact_number')}}:
									{{checkoutInfo.consignee.post_mobile}}</div>
							</div>
							<div class="address_footer" v-if="checkoutInfo.consignee.nearbyleader > 0">{{checkoutInfo.consignee.leader_id ? $t('lang.selected_community') : $t('lang.near_station')}}</div>
						</template>
					</van-cell>
				</van-cell-group>
			</section>
			...
			<section class="checkout-goods-list m-b10 quanymagin" v-for="(item,index) in checkoutInfo.goods_list" :key="index">
				<section class="section-list">
					...
					<van-cell-group class="van-cell-noright m-top08" style="margin-left: -0.4rem;" v-if="item.cross_warehouse_name != ''">
						<van-cell :title="$t('lang.place_of_shipment')" style="padding: 1rem 1.5rem;" class="b-min">
			...
			<section class="checkout-goods-other quanymagin">
				<van-cell-group class="van-cell-noright">
							...
							<template v-else>
								<template v-if="invoice.invoiceType != 1">
									...
									<p class="receipt-title">
                                        ...
                                        <template v-else-if="invoice.invoiceType == 2" >Онлайн чек - </template>
					...
					<!-- 红包 -->
					<van-cell v-model="bonusObject.bonusContent" class="b-min" is-link v-if="bonusList.length>0" @click="bonusHandle">
						<template slot="title"><span>{{$t('lang.bonus')}}({{bonusList.length}}{{$t('lang.zhang')}})</span></template>
					</van-cell>
					
					<!--优惠券-->
					<van-cell v-model="couponsObject.couponsContent" class="b-min" is-link v-if="couponsList.length>0" @click="couponsHandle">
						<template slot="title"><span>{{$t('lang.coupons')}}({{couponsList.length}}{{$t('lang.zhang')}})</span></template>
					</van-cell>

					<!--储值卡-->
					<van-cell v-model="valueCardObject.valueCardContent" value="" class="b-min" is-link v-if="valueCard.length > 0"
				...
				<van-cell-group class="van-cell-noright m-top08" v-if="checkoutInfo.cross_border">
					<van-cell :title="$t('lang.real_name')" class="van-cell-title b-min "></van-cell>
				...
				<van-cell-group class="van-cell-noright m-top08" v-if="checkoutInfo.use_membership_card > 0 && vipCard.membership_card_discount_price > 0">
					<van-cell class="van-cell-title b-min ">
						<template slot="title">
							<div class="vip-zk">
				...
				<van-cell-group class="van-cell-noright m-top08">
					...
					<!--余额-->
					<van-cell class="van-cell-title b-min " v-if="checkoutInfo.use_surplus > 0 && pay_code == 'onlinepay'">
						<div slot="title">{{$t('lang.is_use_balance')}}</div>
						<van-switch v-model="surplusSelf" size="20px" class="fr" />
					</van-cell>
					<van-cell class="b-min " v-if="checkoutInfo.use_surplus > 0 && use_surplus_val > 0 && pay_code == 'onlinepay'">
						<template slot="title">
				...
				<van-cell-group class="van-cell-noright m-top08 van-cell-total">
					<van-cell :title="$t('lang.goods_together')" class="van-cell-title b-min ">
						<div>
							<div class="color-red">
		...
		<van-popup class="show-popup-coupon show-popup-common" round position="bottom" v-model="inventory" :overlay="overlay" closeable>
			...
			<div class="content">
				<div class="goods_thumb">
					...
					<div class="goods_thumb_footer">Тауар бағасына сұрағыңыз бар болса,анығын көруді басыңыз</div>
				</div>
			</div>
		</van-popup>
		...
		<van-popup v-model="showBase" position="right" :overlay="overlay" class="invoice_show">
			<section class="invoice-warp-box padding-all">
				...
				<ul class="ect-selects">
					...
					<li class="ect-select" :class="{'active':invoice.invoiceType == 2}" @click="incrementHandle(1)" v-if="invoice.is_shouqianba > 0"><span>Онлайн чек</span></li>
			...
			<section class="invoice-cont-box" v-if="invoice.invoiceType != 1">
				...
				<div class="invoice-cont-1 m-top08" v-if="invoice.invoiceType == 2">
                    <van-field label="Email：" placeholder="Email енгізіңіз" class="my-cell-nobottom" v-model="invoice.inv_email"/>
                    <van-field label="Тел нөмір：" placeholder="Тел нөмір енгізіңіз" v-model="invoice.inv_mobile"/>
                </div>
				...
				<template v-for="(item, index) in cartList">
					<div class="value-card-list" v-show="currTab == index" v-waterfall-lower="loadMore" waterfall-disabled="disabled" waterfall-offset="300" :key="index">
						...
						<div class="no_data" v-if="shopEmpty">
							<p>Өкінішті!</p>
							<p>{{currTab == 1 ? 'Сізде жарамсыз төлем карта жоқ' : 'Сізде жарамды төлем карта жоқ'}}</p>
						</div>
		...
		<van-popup class="show-popup-common payment-method-popup" v-model="paymentShow" position="bottom">
			...
			<div class="content">
				<div class="tip" :class="{'tipscroll':tipscroll}">
					...
					<div class="txt">Төлем түрін өзгерту төлем сомасына әсер етуі мүмкін,тапсырыс жасаудан бұрын анықтап алыңыз!</div>
		...
		<van-popup class="show-popup-common show-popup-coupon" v-model="couponsObject.couponsBase" position="bottom">
			...
			<div class="content">
				<div class="usable-coupon-number">Жарамды купон ({{couponsList.length}})</div>
				<div class="usable-coupon-money">{{couponsObject.couponsIdArr && couponsObject.couponsIdArr.length}} купон істетілді,жалпы <em class="color-red">{{ currency }}{{couponsObject.couponsMoney}} үнемделді</em></div>
	...
	export default {
		...
		data() {
			return {
				...
				quanyiShow: false,
				back: '',
				shopStore: this.$route.query.type ? this.$route.query.type : '',
				ru_ids: this.$route.query.ru_ids ? this.$route.query.ru_ids : '',
		...
		methods: {
			...
			onClickTab(i) {
				...
				this.loadmoreStatus = this.cartList[i].length == this.cardCount ? 'Басқа' : 'Жүктеуде...';
			...
			shop_kud() {
				if (this.ru_ids && this.ru_ids >= 0) {
					...
					Toast("Бутик тізімін алуда")
					setTimeout(() => {
						//需要延迟的代码 :600毫秒后延迟跳转到首页，可以加提示什么的
						this.$router.push({
							name: 'storeGoods',
							query: {
								rec_id: this.rec_id
			...
			onSubmit() {
				...
				if ((this.pay_code == 'balance' || this.use_surplus_val > 0 || this.valueCardObject.valueCardId > 0) && this.use_paypwd > 0 && this.paypwdValue.length < 6) {
					if(this.use_paypwd_open > 0){
						...
						Dialog.confirm({
			              	message: 'Төлем паролі әлі қосылмады,қосасыз ба?',
				...
				if(this.invoice.invoiceType == 2){
                    ...
                    if (this.invoice.company) {
                        ...
                        if (this.invoice.inv_email == '') {
                            ...
                            Toast('Тел нөмірді дұрыс енгізіңіз')
                            return false
                        }else{
                            this.showBase = false
                        }
...
<style lang="scss" scoped>
	...
	.quanymagin .van-hairline--top-bottom::after{ border-width: 0; }
