<template>
    <div>
        <ec-dialog 
            ...
            <template v-if="tabSelect == 'picture'">
                <div slot="title" class="el-dialog__title">
                    {{$t('lang.my_image')}}
                    <span class="select-num" v-show="this.oneOrMore == 'more'">{{$t('lang.selected')}}
                        <em>{{pictureLength}}</em> {{$t('lang.zhang')}}</span>
                    <a href="javascript:;" @click.stop="updatePicture" class="select-a">{{$t('lang.refresh')}}</a>
                </div>
                <div class="sidebar">
                    <nav>
                        <ul>
                    ...
                    <ul>
                        <li>
                            <a href="javascript:;" @click="returnPcture">
                                <i class="iconfont icon-jiantou12"></i>{{$t('lang.back')}}
                            </a>
                        </li>
                        <li>{{$t('lang.upload_image')}}</li>
                    </ul>
                    <p class="ec-dialog-tips">{{$t('lang.upload_zhi')}}
        ...
        <ec-dialog :visible.sync="dialogVisible">
...
export default {
    ...
    methods: {
        ...
        confirmSelect() {
            ...
            if (this.listIndex || this.listIndex == 0 || this.allValuesType) {
                this.dialogPictureList.forEach(v => {
                    if (v.bActive) {
                        let o = {
                            ...
                            urlCatetory: "",
                            urlName: "",
                            bgColor: localConfig.pageInfo.defalutBg,
                            sort: 1,
                            img: v.pic_file
                        }
                        aSelectPic.push(o)
                ...
                if (localStorage.getItem("aPicture")) {
                    let sLocalPicture = localStorage.getItem("aPicture")
        ...
        thisClose() {
            localStorage.removeItem("aPictureId")
    ...
    computed: {
        ...
        currentPage:{
            get(){
                return Number(this.$store.state.dialogPicture.settingType.currentPage)
            },
            set(val){
                this.$store.state.dialogPicture.settingType.currentPage = val
            }
        },
        sUpload() {
            return `/${window.apiAuthority}/touch_visual/pic_upload`
        },
        ruId(){
