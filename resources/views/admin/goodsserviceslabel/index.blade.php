...
<div class="warpper">
    <div class="title">{{ __('admin/goods_services_label.goods') }} - {{ __('admin/goods_services_label.goods_services_label') }}</div>
    <div class="content">
        <div class="explanation" id="explanation">
            ...
            <ul>
                ...
                @foreach(__('admin/goods_services_label.services_label_list_notice') as $v)
                    <li>{!! $v !!}</li>
                @endforeach
        ...
        <div class="flexilist">
            <div class="common-head">
                <div class="fl">
                    <a href="{{ route('admin/goodsserviceslabel/update', ['type' => $type]) }}">
                        <div class="fbutton">
                            <div class="add " title="{{ __('admin/goods_services_label.add_label') }}"><span><i class="fa fa-plus"></i>{{ __('admin/goods_services_label.add_label') }}</span>
                            </div>
                        </div>
                    </a>
                </div>
                <div class="search">
                    <form action="javascript:search();" method="post" name="searchForm">
                        <div class="select_w140 imitate_select ">
                            ...
                            <ul>
                                <li><a href="javascript:;" data-value="-1">{{ __('admin/goods_services_label.status') }}</a></li>
                                <li><a href="javascript:;" data-value="1">{{ __('admin/goods_services_label.use') }}</a></li>
                                <li><a href="javascript:;" data-value="0">{{ __('admin/goods_services_label.no_use') }}</a></li>
            ...
            <div class="common-content">
                <div class="list-div" id="listDiv">
                    @include('admin.goodsserviceslabel.library.list_query')
                </div>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
    ...
    function toggle_is_show(id, obj) {
        ...
        $.post("{{ route('admin/goodsserviceslabel/update_status') }}", {id: id, val: val}, function (data) {
            return false;
        }, 'json');
    ...
    $(document).on('click', '.batch', function () {
        ...
        if (ids) {
            ...
            if (handler == 'drop') {
                //询问框
                layer.confirm('{{  __('admin/goods_services_label.batch_drop_notice') }} ', {
                    btn: ['{{ __('admin/common.ok') }}', '{{ __('admin/common.cancel') }}'] //按钮
                }, function () {
                    $.post("{{ route('admin/goodsserviceslabel/batch') }}", {handler:handler, id: ids}, function (data) {
                        layer.msg(data.msg);
                        if (data.error == 0) {
                            refresh();
                        }
    ...
    $(document).on('click', "a[ectype='drop']", function () {
        ...
        layer.confirm('{{  __('admin/goods_services_label.batch_drop_notice') }} ', {
            ...
            $.post("{{ route('admin/goodsserviceslabel/drop') }}", {id: id}, function (data) {
                layer.msg(data.msg);
                if (data.error == 0) {
                    refresh();
                }
